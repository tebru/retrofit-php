...
class MethodBodyBuilderTest extends MockeryTestCase
{
    ...
    public function testBodyOptional()
    {
        ...
        $response = $builder->build();
        $expected = '$requestUrl = http://example.com . "/path";$headers = [];if (null !== $body) {$body = $this->serializer->serialize($body, "json");$body = json_decode($body, true);$body = \Tebru\Retrofit\Generation\Manipulator\BodyManipulator::boolToString($body);$body = http_build_query($body);} else { $body = null; }$request = new \GuzzleHttp\Psr7\Request("POST", $requestUrl, $headers, $body);$this->logger->debug("Created Request", ["request" => ["method" => $request->getMethod(), "uri" => (string)$request->getUri(), "headers" => $request->getHeaders(), "body" => (string)$request->getBody()]]);$this->logger->info("Dispatching BeforeSendEvent");$this->eventDispatcher->dispatch("retrofit.beforeSend", new \Tebru\Retrofit\Event\BeforeSendEvent($request));try {$this->logger->info("Sending Synchronous Request");$response = $this->client->send($request->getMethod(), (string)$request->getUri(), $request->getHeaders(), (string)$request->getBody());} catch (\Exception $exception) {$this->logger->error("Caught Exception", ["exception" => $exception]);$this->logger->info("Dispatching ApiExceptionEvent");$this->eventDispatcher->dispatch("retrofit.apiException", new \Tebru\Retrofit\Event\ApiExceptionEvent($exception));throw new \Tebru\Retrofit\Exception\RetrofitApiException(get_class($this), $exception->getMessage(), $exception->getCode(), $exception);}$this->logger->debug("API Response", ["response" => $response]);$this->logger->info("Dispatching AfterSendEvent");$this->eventDispatcher->dispatch("retrofit.afterSend", new \Tebru\Retrofit\Event\AfterSendEvent($response));$return = $this->serializer->deserialize((string)$response->getBody(), "Tebru\Retrofit\Test\Mock\MockUser", "json");$this->logger->info("Dispatching ReturnEvent");$returnEvent = new \Tebru\Retrofit\Event\ReturnEvent($return);$this->eventDispatcher->dispatch("retrofit.return", $returnEvent);return $returnEvent->getReturn();';
    ...
    public function testOptionalCallback()
    {
        ...
        $response = $builder->build();
        $expected = '$requestUrl = http://example.com . "/get";$headers = [];$body = null;$request = new \GuzzleHttp\Psr7\Request("GET", $requestUrl, $headers, $body);$this->logger->debug("Created Request", ["request" => ["method" => $request->getMethod(), "uri" => (string)$request->getUri(), "headers" => $request->getHeaders(), "body" => (string)$request->getBody()]]);$this->logger->info("Dispatching BeforeSendEvent");$this->eventDispatcher->dispatch("retrofit.beforeSend", new \Tebru\Retrofit\Event\BeforeSendEvent($request));try {if ($callback !== null) {$this->logger->info("Sending Asynchronous Request");$response = $this->client->sendAsync($request, $callback);} else {$this->logger->info("Sending Synchronous Request");$response = $this->client->send($request->getMethod(), (string)$request->getUri(), $request->getHeaders(), (string)$request->getBody());}} catch (\Exception $exception) {$this->logger->error("Caught Exception", ["exception" => $exception]);$this->logger->info("Dispatching ApiExceptionEvent");$this->eventDispatcher->dispatch("retrofit.apiException", new \Tebru\Retrofit\Event\ApiExceptionEvent($exception));throw new \Tebru\Retrofit\Exception\RetrofitApiException(get_class($this), $exception->getMessage(), $exception->getCode(), $exception);}$this->logger->debug("API Response", ["response" => $response]);$this->logger->info("Dispatching AfterSendEvent");$this->eventDispatcher->dispatch("retrofit.afterSend", new \Tebru\Retrofit\Event\AfterSendEvent($response));if ($callback !== null) {$this->logger->info("Dispatching ReturnEvent");$returnEvent = new \Tebru\Retrofit\Event\ReturnEvent(null);$this->eventDispatcher->dispatch("retrofit.return", $returnEvent);return $returnEvent->getReturn();}$return = json_decode((string)$response->getBody(), true);$this->logger->info("Dispatching ReturnEvent");$returnEvent = new \Tebru\Retrofit\Event\ReturnEvent($return);$this->eventDispatcher->dispatch("retrofit.return", $returnEvent);return $returnEvent->getReturn();';
